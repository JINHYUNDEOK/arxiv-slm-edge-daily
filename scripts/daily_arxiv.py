...
ARXIV_CATEGORY_LABELS = {
    ...
    "format_lora": "Format-LoRA/Structured Output",
...
핵심 목표:
SLM, small language model, edge device, on-device, TinyML 환경에서 LoRA, QLoRA, PEFT, adapter fine-tuning을 이용해 특정 출력 형식, structured output, JSON/schema, function calling, tool calling, chat template, instruction format, response format을 잘 따르게 만드는 논문을 우선 선별한다.
...
3. LoRA, QLoRA, PEFT, adapter fine-tuning, supervised fine-tuning, instruction tuning 중 하나 이상과 관련
4. structured output, JSON output, JSON schema, schema following, function calling, tool calling, chat template, instruction format, response format, prompt format처럼 출력 형식 준수와 관련 있으면 우선
5. 실제 엣지 배포, CPU 배포, GGUF/ONNX 변환, 양자화, 메모리 절감, 추론 지연시간 감소와 관련 있으면 우선
